...
public class JedisUtil {
	...
	public JedisUtil() {
		...
		JedisShardInfo shard = new JedisShardInfo(JEDIS_POOL, JEDIS_PORT,
				"master");
		list.add(shard);
	 ...
	 * @return
	 */
	public boolean setHSet(byte[] domain, byte[] key, byte[] value) {
		if (value == null)
			return false;
		ShardedJedis shardedJedis = null;
		try {
	 ...
	 * @return Json String or String value
	 */
	public byte[] getHSet(byte[] domain, byte[] key) {
		ShardedJedis shardedJedis = null;
		try {
			shardedJedis = shardedJedisPool.getResource();
			return shardedJedis.hget(domain, key);
	 ...
	 * @return 删除的记录数
	 */
	public long delHSet(byte[] domain, byte[] key) {
		ShardedJedis shardedJedis = null;
		long count = 0;
		try {
			shardedJedis = shardedJedisPool.getResource();
	 ...
	 * @return
	 */
	public boolean existsHSet(byte[] domain, byte[] key) {
		ShardedJedis shardedJedis = null;
		boolean isExist = false;
		try {
			shardedJedis = shardedJedisPool.getResource();
